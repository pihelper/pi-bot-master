...
class Shop:
        ...
        shipping_data = {
            ...
            'variables': {
                ...
                "taxes": {
                    ...
                    "proposedTotalAmount": {
                        "value": {
                            "amount": graphql_json['session']['negotiate']['result']['sellerProposal']['tax'][
                                tax_key]['value']['amount'],
                            "currencyCode": graphql_json['session']['negotiate']['result']['sellerProposal']['tax'][
    ...
    def fix_graphql(self, req):
        start = str(req.text).split('<div data-serialized-id="graphql" data-serialized-value="')[1]
        end = str(start).split('</div>')[0]
        to_parse = end.replace('\\&quot;', '"').replace('&quot;', '"').replace('\\"', '"').replace('"\\">"',
                                                                                                   '').replace('">',
                                                                                                               '').replace(
            '{"sessionToken":""}', '')
        final_json = str(self.delete_between_strings(to_parse, '"postPurchaseAdditionalTrackingScript"',
                                                     '}\\n&lt;/script&gt;",'))
        new_final = final_json.replace('{"checkpointData"', '":{"checkpointData"').replace('{"basic-card-v1":{}}',
                                                                                           '').replace('":{"session"',
                                                                                                       ',"session"', 1)
        new_new = self.delete_between_strings(new_final[:-1], '"translations"', '"}}",')
        return json.loads(new_new)
